...
class SamplePlugin implements Plugin {
	...
	public function activate(context:PluginContext):Void {
		activations++;
		context.addCommand("sample:run", function(editor:CommandContext) { this.performed++; });
...
class PluginTestMain {
	...
	static function main():Int {
		...
		var window = Native.window_create("plugin-test", 320, 200), renderer = new Renderer(window, "ignored-headlessly.ttf", 15),
			application = new Application(renderer, 320, 200), plugin = new SamplePlugin();
		...
		require(plugin.deactivations == 1 && !application.commands.contains("sample:run")
			&& !application.keyPressed(77, 3), "plugin registrations survived unload");
		...
		require(failed && !application.plugins.isLoaded("broken") && !application.commands.contains("broken:leak"),
			"failed activation leaked plugin state");
		...
		Native.shutdown();
		Sys.println("PASS: plugin activation, dispatch, rollback, reload, and cleanup");
		return 0;
	}
}
